...
VI discard_hand(int_fast32_t h) {


    VP hand = decode_hand(h);
    VI discard(0);
    ...
    if ((bits / (bits & -bits) == 31) || (bits == 0x100F)) {
        ...
        discard.push_back(hand[1]);
        discard.push_back(hand[2]);
        return discard;
    } else {
        int_fast32_t i = 0, p1 = 0, p2 = 0, t = -1, hc = -1;
        ...
        if (p1) {
            if (p2) {
                for (int i = 0; i != 5; ++i)
                {
                    if(hand[i].first != p1 && hand[i].first != p2) {
...
int main() {
    ...
    int rank = rank_hand(encode_hard(cards));
}
